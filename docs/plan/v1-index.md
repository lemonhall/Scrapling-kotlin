...
- M4 已锁定 `Playwright Java + Chromium` 方案，当前已落地 sync/async 浏览器抓取基线、真实 page reuse、browser request options 语义测试、stealth launch parity、pageAction、Cloudflare 检测与基础求解 flow、`timeout/wait/retries`。
- 解析层当前已完成 M1 + M2；M3 Static Fetchers 已完成，M4 Browser Fetchers 当前剩余连接/启动上下文/导航响应对齐工作。

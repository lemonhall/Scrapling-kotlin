...
class AsyncFetcherSession(
    private val transport: AsyncHttpTransport = AsyncJdkHttpTransport.sessionTransport,
    timeout: Int = 30,
    retries: Int = 3,
    ...
    fun open(): AsyncFetcherSession {
        check(!isOpen) { "AsyncFetcherSession is already open." }
        isOpen = true
        return this
    }

    suspend fun get(url: String, options: RequestOptions = defaultOptions()): Response = request("GET", url, options)
    ...
    suspend fun delete(url: String, options: RequestOptions = defaultOptions()): Response = request("DELETE", url, options)

    private suspend fun request(method: String, url: String, options: RequestOptions): Response {
        check(isOpen) { "AsyncFetcherSession must be opened before making requests." }
